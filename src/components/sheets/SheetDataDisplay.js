...
const ConfigurationSheet = ({ sheet, realTimeData }) => {
  ...
  const ruleNamesArray = useCallback(() => {
    ...
    try {
      ...
      Object.entries(realTimeData).forEach(([key, value]) => {
        ...
        if (keyParts[7] !== "cov") {
          ...
          newTotalByManyConf[totalByManyConfIdentifier].totalSellSheet +=
            sellVol;
          ...
          newTotalByManyConfCov[totalByManyConfIdentifier].totalBuySheet +=
            buyVol;
          newTotalByManyConfCov[totalByManyConfIdentifier].totalSellSheet +=
            sellVol;
        ...
        <Table>
          ...
          <TableBody>
            ...
            {sheet?.dealer_configurations?.map((config, index) => {
              const buyIdentifier = `${sheet.sheet_name}:${sheet.sheet_id}:${config.server_id}:${config.Login}:${config.manager_id}:${config.SymbolSuffix}:${config.MainSymbol}:buy`;
              const sellIdentifier = `${sheet.sheet_name}:${sheet.sheet_id}:${config.server_id}:${config.Login}:${config.manager_id}:${config.SymbolSuffix}:${config.MainSymbol}:sell`;
              ...
              return (
                <React.Fragment key={`conf-${index}`}>
                  {totalTopConfFiltered[index] !== "" && (
                    <MemoizedTableRow
                      ...
                      <TableCell>
                        <KeyboardArrowDown
                          onClick={() =>
                            showCollapse(totalTopConfFiltered[index])
                          }
                          sx={{ cursor: "pointer" }}
                        />
                      </TableCell>
                  ...
                  {showCollapseRows[index]?.isDisplayed && (
                    <MemoizedTableRow>
                      ...
                      <TableCell>
                        {toFixedIfNeeded(sellVol, DECIMAL_POINTS)}
                      </TableCell>
                      <TableCell>
                        {toFixedIfNeeded(netVol, DECIMAL_POINTS)}
                      </TableCell>
            ...
            >
              ...
              {rulesName.map((_, ruleIndex) => {
                const coveredResult =
                  ...
                  (totalByManyConfCov[`${sheet.sheet_id}tbmc`]?.totalResult[
                    ruleIndex
                  ] || 0);
    ...
    <Box>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          {sheets[selectedTab]?.creator_dealer_id === userId && (
