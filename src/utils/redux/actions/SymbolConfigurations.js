...
export const GetMT5SymbolConfigurations = () => {
    return async (dispatch) => {
      try {
        ...
        const response = await axios.get(`${apiIp}:${apiPort}${API_MT5SYMBOLCONFIGURATION}`, {
          headers: {
       ...
       const data =JSON.parse(response.data);
        ...
        dispatch({ type: GET_MT5SYMBOLCONFIGURATIONS_FAILURE, payload: errorPayload });
  ...
  export const GetMT5Suffixes = () => {
    return async (dispatch) => {
      try {
        ...
        const apiPort = process.env.REACT_APP_API_PORT;
        const authorizationToken = localStorage.getItem(ATFXTOKEN);
        const response = await axios.get(`${apiIp}:${apiPort}${API_MT5SUFFIXES}`, {
          headers: {
            Authorization: `${authorizationToken}`,
          },
          timeout: GLOBAL_REQUEST_TIMEOUT,
